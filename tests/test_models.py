...
def test_registry_uses_modelscope_first(tmp_path):
    ...
    assert "fallback" in next(item for item in status if item["id"] == "lama")["provider"].lower()
...
def test_lama_download_is_checksum_validated_and_atomic(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "manga_localizer.model_manager.urlopen", lambda *_args, **_kwargs: Response(payload)
    )
    manager = ModelManager(paths(tmp_path))
